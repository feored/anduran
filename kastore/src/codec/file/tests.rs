...
#[test]
fn decode_file_rejects_body_raw_size_above_parser_limit() {
    let (mut bytes, body_offset) = minimal_file_bytes_with_body_offset(
        SaveVersion::FORMAT_VERSION_1111_RELEASE,
        b"10032",
        None,
    ...
    bytes[body_offset..body_offset + 4].copy_from_slice(&(300 * 1024 * 1024u32).to_be_bytes());
    ...
    assert_eq!(
        ...
        crate::Error::Parse(ParseError {
            ...
            offset: body_offset,
            kind: ParseErrorKind::InvalidValue {
                message: "body raw size exceeds parser limit",
            },
        })
    );
}
