...
const MAX_BODY_RAW_SIZE: usize = 256 * 1024 * 1024;
const MAX_BODY_ZIP_SIZE: usize = 256 * 1024 * 1024;
...
pub(crate) fn encode_map_info(
    ...
    writer.write_save_string(&file_info.filename, "map filename")?;
    writer.write_save_string(&file_info.name, "map name")?;
    writer.write_save_string(&file_info.description, "map description")?;
    ...
    if revision == MapInfoRevision::V10033 {
        match &file_info.creator_notes {
            Some(creator_notes) => writer.write_save_string(creator_notes, "creator notes")?,
...
fn decode_body_bytes(
    ...
    if raw_size > MAX_BODY_RAW_SIZE {
        return Err(reader.invalid_value(
            "body raw size",
            raw_size_offset,
            "body raw size exceeds parser limit",
        ));
    }
    ...
    if zip_size > MAX_BODY_ZIP_SIZE {
        return Err(reader.invalid_value(
            "body zip size",
            zip_size_offset,
            "body zip size exceeds parser limit",
        ));
    }
...
fn encode_body_bytes(
    ...
    if body.len() > MAX_BODY_RAW_SIZE {
        return Err(Error::InvalidModel {
            field: "body",
            message: "body length exceeds encoder limit",
        });
    }
    ...
    if zlib_bytes.len() > MAX_BODY_ZIP_SIZE {
        return Err(Error::InvalidModel {
            field: "body",
            message: "compressed body length exceeds encoder limit",
        });
    }
