...
#[test]
fn save_rejects_world_tile_count_mismatch() {
    let mut save_game = load_fixture();
    save_game.world.tiles.pop();
    ...
    assert_eq!(
        ...
        Error::InvalidModel {
            field: "world tiles",
            message: "world tile count must match width * height",
        }
    );
}

#[test]
fn save_rejects_header_world_dimension_mismatch() {
    let mut save_game = load_fixture();
    save_game.header.file_info.width += 1;
    ...
    assert_eq!(
        ...
        Error::InvalidModel {
            ...
            message: "header map dimensions must match world dimensions",
        }
    );
}

#[test]
fn save_rejects_settings_game_type_mismatch() {
    let mut save_game = load_fixture();
    save_game.settings.game_type.insert(GameType::CAMPAIGN);
    ...
    assert_eq!(
        error,
        Error::InvalidModel {
            field: "settings game type",
            message: "header game type must match settings game type",
        }
    );
}

#[test]
fn save_rejects_campaign_data_for_non_campaign_game() {
    let mut save_game = load_fixture();
    save_game.campaign_save_data = Some(CampaignSaveData::default());
    ...
    assert_eq!(
        ...
        Error::InvalidModel {
            ...
            message: "non-campaign game type must not include campaign save data",
        }
    );
}
