...
use crate::model::validation::validate_kingdoms;
...
pub(crate) fn decode(reader: &mut Reader<'_>) -> std::result::Result<World, Error> {
    ...
    let tiles_count = reader.read_len_u32("world tiles count")?;
    let mut tiles: Vec<Tile> = Vec::with_capacity(tiles_count);
...
pub(crate) fn encode_into(writer: &mut Writer, world: &World) -> std::result::Result<(), Error> {
    ...
    writer.write_len_u32(world.tiles.len(), "world tiles")?;
