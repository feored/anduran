...
pub(super) fn decode(
    ...
    let count = reader.read_len_u32("map objects count")?;
...
pub(super) fn encode(
    ...
    writer.write_len_u32(map_objects.len(), "world map objects")?;
...
fn encode_event(writer: &mut Writer, event: &MapEvent) -> std::result::Result<(), Error> {
    ...
    writer.write_save_string(&event.message, "map event message")?;
...
fn decode_sphinx(reader: &mut Reader<'_>) -> std::result::Result<MapSphinx, Error> {
    ...
    let answers = reader.read_vec_u32("map sphinx answers count", |reader| {
        reader.read_save_string("map sphinx answer")
    })?;
...
fn encode_sphinx(writer: &mut Writer, sphinx: &MapSphinx) -> std::result::Result<(), Error> {
    ...
    writer.write_len_u32(sphinx.answers.len(), "world map sphinx answers")?;
    for answer in &sphinx.answers {
        writer.write_save_string(answer, "map sphinx answer")?;
    }
    writer.write_save_string(&sphinx.riddle, "map sphinx riddle")?;
...
fn encode_localized_string(
    writer: &mut Writer,
    value: &LocalizedString,
) -> std::result::Result<(), Error> {
    writer.write_save_string(&value.text, "localized string text")?;
