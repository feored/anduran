...
pub(super) fn decode(reader: &mut Reader<'_>) -> std::result::Result<Vec<Castle>, Error> {
    reader.read_vec_u32("castles count", decode_castle)
}

pub(super) fn encode(writer: &mut Writer, castles: &[Castle]) -> std::result::Result<(), Error> {
    writer.write_len_u32(castles.len(), "world castles")?;
...
fn decode_castle(reader: &mut Reader<'_>) -> std::result::Result<Castle, Error> {
    ...
    let mage_guild_normal_spells =
        reader.read_vec_u32("castle mage guild normal spells count", |reader| {
            Ok(Spell::from_i32(
                reader.read_i32_be("castle mage guild normal spell")?,
            ))
        })?;
    let mage_guild_library_spells =
        reader.read_vec_u32("castle mage guild library spells count", |reader| {
            Ok(Spell::from_i32(
...
fn encode_castle(writer: &mut Writer, castle: &Castle) -> std::result::Result<(), Error> {
    ...
    writer.write_save_string(&castle.name, "castle name")?;
